...
import com.ecaray.basicres.entity.AreaAndSectionListBean;
import com.ecaray.basicres.entity.UpLoadLocationBean;
import com.ecaray.basicres.data.LoginData;
import com.ecaray.basicres.entity.VersionBean;
...
public class PubDataCenter {
    ...
    public Flowable<UpLoadLocationBean> uploadTrack(double longitude, double latitude, String placeName){
        ...
        String lMethod = "uploadTrack";
        TreeMap<String, String> lTreeMap = HttpParam.getNormalParamsMapWithU(lMethod);
    ...
    /**
     * 14.上传图片接口
     * description 签到 type="pdasignin" 签出 type="pdasignout" 申请订单 type="startparking" 违章上报type="IllegalImg"
     */
     ...
     * 获取片区路段列表
     */
    public Flowable<AreaAndSectionListBean> getAreaAndSectionList(){
        String lMethod = "getAreaAndSectionList";
        TreeMap<String, String> lTreeMap = HttpParam.getNormalParamsMapWithU(lMethod);
        lTreeMap.put("comid", LoginData.getInstance().getComId());
        lTreeMap.put("userid", LoginData.getInstance().getU());
        TreeMap<String , String> lEncTreeMap = HttpParam.securityKeyMethodEnc(lTreeMap);
        //"这里是模拟文件的内容"
        return sPubService.getAreaAndSectionList(lEncTreeMap);
    }
